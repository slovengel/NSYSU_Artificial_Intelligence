...
void output(int i,int j,int color,int ant,int life);
...
int main(){
    ...
    for(int i=0; i<SIZE; i++){          //設定並輸出初始狀態
		for(int j=0; j<SIZE; j++){
            ...
            output(i,j,color1[i][j],ant1[i][j],0);
    ...
    while(kbhit()==0){      //按任意鍵暫停
		...
		for(int i=0; i<SIZE; i++){
			for(int j=0; j<SIZE; j++){
                ...
                if(((i==SIZE/2-1&&j==SIZE/2-1)||(i==SIZE/2-1&&j==SIZE/2+1)||(i==SIZE/2+1&&j==SIZE/2-1)||(i==SIZE/2+1&&j==SIZE/2+1)) && (ant1[i][j]=='L' || ant1[i][j]=='U'|| ant1[i][j]=='R'|| ant1[i][j]=='D')){   //找到食物的螞蟻回到巢
                    ...
                    if(timeinterval1[i][j]==80||timeinterval1[i][j]==160){   //換一種新的移動策略
                        do{
                            a=rand()%3+1;
                        }while(a==search1[i][j]);
                        search1[i][j]=a;
		...
		for(int i=0; i<SIZE; i++){
            for(int j=0; j<SIZE; j++){
				...
				output(i,j,color2[i][j],ant2[i][j],timeinterval2[i][j]);    //輸出新狀態
...
void output(int i,int j,int color,int ant,int life){			//輸出畫面
